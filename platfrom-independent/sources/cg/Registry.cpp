...
#define CONNECT_ENTITY(entity, NOTIFIER, LISTENER)      \
	{                                                   \
		...
		if (l)                                          \
		{                                               \
			NOTIFIER::instance()->add(l);               \
		}                                               \
	}

	inline void Registry::connectEntity(Entity *entity)
	{
		...
		CONNECT_ENTITY(entity, DrawNotifier, IDrawListener)
		CONNECT_ENTITY(entity, DrawOverlayNotifier, IDrawOverlayListener)
...
#define DISCONNECT_ENTITY(entity, NOTIFIER, LISTENER)      \
	{                                                      \
		...
		if (l)                                             \
		{                                                  \
			NOTIFIER::instance()->remove(entity->getId()); \
		}                                                  \
	}

	inline void Registry::disconnectEntity(Entity *entity)
	{
		...
		DISCONNECT_ENTITY(entity, DrawNotifier, IDrawListener)
		DISCONNECT_ENTITY(entity, DrawOverlayNotifier, IDrawOverlayListener)
...
#define DUMP_ENTITY(entity, LISTENER, tab)              \
	{                                                   \
		...
		if (l)                                          \
		{                                               \
			file << tab << #LISTENER << std::endl;      \
		}                                               \
	}

	inline void Registry::dumpEntity(Entity *entity, std::ofstream &file)
	{
		entity->dump(file);
		file << std::endl;
		std::string tab = "    ";
