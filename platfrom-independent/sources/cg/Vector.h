...
#define for_each_i(X)           \
	...
	public:
		...
		Vector(){for_each_i(data[i] = 0)} Vector(const T x){for_each_i(data[i] = x)} Vector(const T v[N]){for_each_i(data[i] = v[i])} Vector(const V &v) { for_each_i(data[i] = v.data[i]) }
		...
		{
			for_each_i(if (data[i] != k) return false;) return true;
		}
		bool operator==(const V &v) const
		{
			for_each_i(if (data[i] != v.data[i]) return false;) return true;
		}
		bool operator!=(const T k) const
		{
			for_each_i(if (data[i] != k) return true;) return false;
		}
		bool operator!=(const V &v) const
		{
			for_each_i(if (data[i] != v.data[i]) return true;) return false;
		}
		friend const bool isEqual(const V &v0, const V &v1, T threshold)
		{
			for_each_i(if (fabs((double)v0.data[i] - (double)v1.data[i]) > (double)threshold) return false;) return true;
		}
		...
		{
			V v_new;
			for_each_i(v_new.data[i] = -data[i];) return v_new;
		}
		...
		{
			V v_new;
			for_each_i(v_new.data[i] = data[i] + v.data[i];) return v_new;
		}
		const V operator-(const V &v) const
		{
			V v_new;
		...
		{
			V v_new;
			for_each_i(v_new.data[i] = data[i] * k;) return v_new;
		}
		const V operator/(const V &v) const
		{
			V v_new;
		...
		{
			T result = 0;
			for_each_i(result += v.data[i] * v.data[i];) return result;
		}
		friend const double length(const V &v)
		{
			double result = 0;
			for_each_i(result += v.data[i] * v.data[i];) return sqrt(result);
		}
		...
		{
			T result = 0;
			for_each_i(result += v1.data[i] * v2.data[i];) return result;
		}
	...
	{
		V v_new;
		for_each_i(v_new.data[i] = v.data[i] * k;) return v_new;
	}
	template <class V, class T, int N>
